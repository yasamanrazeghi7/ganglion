class TimeSlotsController < ApplicationController
  ...
  # GET /time_slots.json
  def index
    @time_slots = @experiment.time_slots
  end
  ...
  def create
    @time_slot = @experiment.time_slots.new(time_slot_params)
    # @time_slot = TimeSlot.new(time_slot_params)

    respond_to do |format|
      if @time_slot.save
        format.html { redirect_to experiment_time_slot_path(@experiment, @time_slot), notice: 'Time slot was successfully created.' }
  ...
  private
    ...
    def time_slot_params
      params.require(:time_slot).permit(:appointment_time, :researcher_id, :status, :subject_id)
    end
end
